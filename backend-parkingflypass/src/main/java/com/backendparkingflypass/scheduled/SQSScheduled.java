...
@Component
public class SQSScheduled extends TaskSchedule {
    @Autowired
    private final TransactionService transactionService;
    @Autowired
    private final SqsService sqsService;

    protected SQSScheduled(TransactionService transactionService, SqsService sqsService) {
        super();
    ...
    protected void processTask() {
        List<Message> messages = sqsService.getMessages(AWSClient.TRANSACTION_SQS_URL);
        List<Message> messagesToDelete = transactionService.createTransaction(messages);
        sqsService.deleteMessages(messagesToDelete, AWSClient.TRANSACTION_SQS_URL);
    }
}
